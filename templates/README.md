<!--
  ...
  delete the guidance comments. The filled reference is `tree/README.md` (single
  command) and `flac/README.md` (multicall). Keep it tight — intro + Usage are
  for end users and must be jargon-free; mechanism/quirks go in Build notes.
-->
# <pkg>

Standalone build of [<pkg>](<upstream-homepage>)<optional: — one-line description of what it does>.
...
<!--
  Technical detail lives HERE, never in Usage: the multicall / argv[0] dispatch
  mechanism, disabled upstream features, per-platform specifics, and — in one
  line — why a platform is excluded (the upstream gap). No roadmaps, porting
  plans, or effort estimates: we package what we ship, we don't plan upstream
  development. Drop this section if there's nothing to note.
-->
