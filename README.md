...
- [embedded-man.md](embedded-man.md) — how `unpin man` works: the roff is embedded per package (killing the man-only data tarballs), and `unpin man` is a builtin that renders it in-process via the vendored render-only `mandoc-sys` crate, paged by `unpin/src/render/`.
