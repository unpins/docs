...
- [helper-verbs.md](helper-verbs.md) — the durable model for `unpin`'s verb commands (`man`, and `readme`/`search`/… later): each is a `unpins/unpin-<verb>` package reached only via `unpin <verb>`, never on `PATH`, with the dispatch precedence and catalog naming reservation that stop the verb name from colliding with the OS or the catalog.
