...
`engine = "unpin-llvm"` swaps the build toolchain: instead of nixpkgs' gcc-based static stdenv, the package **and its whole static dependency closure** compile with the project's clang/LLVM toolchain to LLVM bitcode, linked by lld. This is the catalog default in practice. Off it, measured over the whole eval matrix (1044 targets, 2026-09-21): the four Rust packages (`cfonts`, `fish`, `rsvg-convert`, `unpin` — the first goes through `mkRustCrate`, not `mkStandaloneFlake`) on every target; `busybox` on linux, because kbuild pipes every `-MD` depfile through `fixdep`, which reopens headers the engine serves from a virtual root with no on-disk existence (its flake says so at length); and `xvfb` and `xvnc` on darwin. On linux those two set no `engine` and are on it all the same, through a bespoke `build` that calls `lib.enginePkgsStaticFor` directly. On Windows the engine is the mingw path only where `multicall.windows = true` (see *Scope* below): 29 `.exe`s are cosmo by design, and eleven still build with nixpkgs' mingw gcc — `biber`, `curl`, `fpcalc`, `gvim`, `openssl`, `perl`, `php`, `python`, `sox`, `usbutils`, `vim`.

**Measure adoption by the compiler, not by the text.** A target is on the engine when the derivation that compiles the package has a stdenv whose closure contains `unpin-cc-unwrapped`. Both shortcuts give the wrong answer: grepping a flake for `engine` misses `xvfb`/`xvnc`, and grepping a closure for `unpin-llvm` counts targets that are off, because `unpin-llvm` is also the tool that writes the embedded ZIP. The package's own derivation is usually NOT the top one — that is the embed wrapper — and may not carry the package's name (`python3-static-…`, `x264-static-…`).

An engine target can still link a dependency built outside it. Measured the same way (target-triple derivations with a non-engine stdenv, the toolchain's own closure excluded): nix-lib's own `unpin-dns-fallback` on the linux targets of the 12 `dnsFallback` packages; the librsvg/cairo/pango/glib stack in `chafa` and `ffmpeg`; nixpkgs' `libcxx` on darwin (`chafa`, `dec265`, `ffmpeg`, `jbig2`, `nmap`, `x265`); boost in `poppler-utils` on the crosses; the X libraries under `xvfb`/`xvnc`'s in-process xkbcomp on the crosses. Some of these edges may be build-only inputs — check the link line before acting on one. A nixpkgs `musl-static` or `*-gfortran` in the list is the gcc toolchain riding in the closure, not a second libc in the binary.
