...
- **Follow nixpkgs.** We keep no security posture of our own: ours is the `nixos-YY.MM` channel `nix-lib` pins, re-pinned monthly. Only `nix-lib` declares a `nixpkgs` URL — a package reaches it via `follows`, never a second `url =`. **Build from the nixpkgs derivation** (`overrideAttrs`, or a package's own supported `.override`): a fresh `mkDerivation` over `pkgs.<attr>.src` drops the whole recipe — `patches`, `postPatch`, `postUnpack`, `configureFlags` — and that is how a security backport misses us in silence (it hid an unpatched musl behind 107 packages until 2026-09-24). Nothing enforces this in CI yet, so it is audited at each re-pin against the divergence register. When we are behind, ask upstream to backport; a local override is the last resort and must be registered. See [upstream-policy.md](upstream-policy.md).
...
- [upstream-policy.md](upstream-policy.md) — what we pin and why the channel not the branch, re-pin cadence and ordering, how nixpkgs backports into a stable release, how to measure exposure (and which measurements lie), the rule against building from `src`, the divergence register.
