...
- [embedded-man.md](embedded-man.md) — `unpin man` is a **builtin** that renders embedded roff in-process via the `mandoc-sys` crate (vendored render-only mandoc, linked), paged by `unpin/src/render/`; the `unpin bundle list|dump` interface stays for independent verb-packages.
